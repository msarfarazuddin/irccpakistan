...
function Chevron({ open }: { open: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={`h-8 w-8 text-[#1e76bd] transition-transform sm:h-7 sm:w-7 ${open ? "rotate-180" : "rotate-0"}`}
      fill="none"
      aria-hidden="true"
    >
...
function FaqItem({
  ...
  return (
    <div className="rounded-md bg-white shadow-sm ring-1 ring-black/5">
      ...
      >
        <span className="text-[18px] font-semibold leading-snug text-[#1e76bd] ">{q}</span>
        <Chevron open={isOpen} />
      </button>
