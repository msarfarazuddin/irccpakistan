import Image from "next/image";
import LazyBackgroundVideo from "@/app/components/media/LazyBackgroundVideo";


const symptoms = [
    {
        title: "Urinary incontinence, which can range from some leaking to complete loss of bladder control",
        img: "/one1.png",
    },
    {
        title: "Irritative voiding symptoms",
        img: "/two2.png",
    },
    {
        title: "Increased urinary frequency, urgency, and pain upon urination",
        img: "/three.png",
    },

];

export default function FibroidSymptomsSection() {
    return (
        <section className="relative  bg-[#0E58A8E5] from-gray-100 to-gray-200 py-16  px-4"  >
            <LazyBackgroundVideo
                src="/banner-cand.mp4"
                className="absolute top-0 left-0 w-full h-full object-cover"
            />

            {/* Blue Overlay */}
            <div className="absolute inset-0 bg-[rgba(14,88,168,0.9)]"></div>


            <div className="relative  max-w-[1400px] mx-auto text-center">

                {/* Top Badge */}

                <div className="inline-flex w-fit items-center gap-2 rounded-full shadow-lg border border-[#dce6ff] bg-white px-4 py-2 text-sm font-semibold text-[#0E58A8]">
                    <span className="flex h-6 w-6 items-center font-bold justify-center rounded-full text-[30px]  font-bold text-[#0E58A8]">
                        +
                    </span>
                    <span className="text-[#111111] font-normal">Symptoms</span>
                </div>

                {/* Heading */}
                <h5 className="text-3xl text-white md:text-4xl pt-7 mb-4">
                    Why have
                    <span className="text-white font-bold"> Prostatic Artery Embolization?</span>
                </h5>

                <p className="text-white max-w-4xl mx-auto mb-2 text-sm md:text-base">
                    As the prostate gets bigger, it may constrict or partly block the urethra, causing lower urinary tract symptoms such as:
                </p>


                {/* Cards Grid */}

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {symptoms.map((item, index) => {
                        const isSecondLast = index === symptoms.length - 2;
                        const isLast = index === symptoms.length - 1;

                        return (
                            <div
                                key={index}
                                className={`
                bg-[#386fab] w-full max-w-full mx-auto rounded-xl shadow-xl 
                hover:shadow-md transition duration-300 p-4
                ${isSecondLast ? "lg:col-start-2" : ""}
                ${isLast ? "lg:col-start-3" : ""}
            `}
                            >

                                {/* <div className="relative w-full h-48 rounded-lg overflow-hidden">
                                    <Image
                                        src={item.img}
                                        alt={item.title}
                                        fill
                                        className="object-cover"
                                    />
                                </div> */}
                                <h3 className="mt-4 text-[18px] md:text-base font-semibold pl-8 pr-8 text-white text-center">
                                    {item.title}
                                </h3>
                            </div>
                        );
                    })}
                </div>

                <p className="text-white max-w-4xl mx-auto mt-5 text-sm md:text-base">
                    For many patients, these symptoms significantly interfere with daily quality of life. Men experiencing vascular or circulatory issues alongside BPH may also benefit from learning about <a href="/Peripheral-Vascular-Disease" > Peripheral Vascular Disease</a> treatment options available at IRCC Pakistan.
                </p>

            </div>
        </section>
    );
}
