...
export default function AboutSection() {
  const videoIds = videos
    .map((video) => getYouTubeId(video.url))
    .filter((id): id is string => Boolean(id));

  const [active, setActive] = useState(0);
  const total = videoIds.length;

  const prev = () => setActive((index) => (total ? (index - 1 + total) % total : 0));
  const next = () => setActive((index) => (total ? (index + 1) % total : 0));
            ...
            <div className="rounded-2xl bg-[#f4f8ff] p-6 sm:p-8">
              ...
              <div className="aboutlogo mt-6 flex flex-wrap items-center gap-x-8 gap-y-4">
                <div className="relative h-20 w-25">
                  <Image src="/geo.png" alt="Geo" fill sizes="100px" className="object-contain" />
                </div>
                <div className="relative h-20 w-25">
                  <Image src="/capital.png" alt="Capital TV" fill sizes="100px" className="object-contain" />
                </div>
                <div className="relative h-20 w-25">
                  <Image src="/metro.png" alt="Metro" fill sizes="100px" className="object-contain" />
                </div>
                <div className="relative h-20 w-25">
                  <Image src="/atv.png" alt="ATV" fill sizes="100px" className="object-contain" />
                </div>
                <div className="relative h-20 w-25">
                  <Image src="/dawn.png" alt="Dawn" fill sizes="100px" className="object-contain" />
                </div>
                <div className="relative h-20 w-25">
                  <Image src="/sama.png" alt="Samaa" fill sizes="100px" className="object-contain" />
                </div>
              </div>
            </div>
          </div>
